...
Non, le code n'est pas structuré en couches, tout se trouve dans le controller ToDoListController. C'est un code monolithique, qui pose donc des problèmes pour maintenir l'application et la tester, car si on change la base de données ou la logique métier, on est obligé de modifier le code.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Le code métier se trouve dans la méthode `finalContent()` du `TodoListController`. Elle gère la règle des 24h, qui indique que si un item a plus de 24h, on ajoute dans le contenu une note "[LATE!]".

*Cette règle est-elle facilement testable par un test unitaire ?*

Non, elle n'est pas facile à tester car elle est private et enfermée dans un Controller. Pour la tester, il faudrait démarrer tout un contexte Web, ce qui est beaucoup plus lent qu'un test unitaire alors que c'est juste pour un simple calcul.
