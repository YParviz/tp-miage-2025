...
Elouan Rousseau et Youssef Parviz
...
**Réponse :** Non, le code n'est pas structuré en couches, tout se trouve dans le controller ToDoListController. C'est un code monolithique, qui pose donc des problèmes pour maintenir l'application et la tester, car si on change la base de données ou la logique métier, on est obligé de modifier le code.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

**Réponse :** Le code métier se trouve dans la méthode `finalContent()` du `TodoListController`. Elle gère la règle des 24h, qui indique que si un item a plus de 24h, on ajoute dans le contenu une note "[LATE!]".

*Cette règle est-elle facilement testable par un test unitaire ?*

**Réponse :** Non, elle n'est pas facile à tester car elle est private et enfermée dans un Controller. Pour la tester, il faudrait démarrer tout un contexte Web, ce qui est beaucoup plus lent qu'un test unitaire alors que c'est juste pour un simple calcul.
...
* `-it` : Mode interactif avec terminal (permet de voir les logs de la base en direct).
* `-e POSTGRES_PASSWORD=password` : Définit une variable d'environnement pour le mot de passe de la base.
* `-p 5432:5432` : Redirige le port 5432 du PC vers le port 5432 du conteneur.
...
**Réponse :** Il est `null` parce que Spring ne sait pas quel constructeur utiliser. La meilleure façon est l'injection par constructeur, car cela garantit que l'objet est toujours créé avec ses dépendances et cela facilite les tests unitaires.
...
**Réponse :** L'architecture hexagonale a pour but d'isoler le code métier du reste de l'application (base de données, interface Web, framework).

Voici les 4 grands principes :

* Le coeur métier isolé : contient toute la logique et les règles de l'application. Il est totalement pur et indépendant des technologies externes (pas de framework, pas de base de données, pas de HTTP).
* Les ports (contrats) : Ce sont de simples interfaces définies par le domaine. Elles dictent comment communiquer avec lui (ce qu'il offre ou ce dont il a besoin) sans se soucier de la réalisation technique.
* Les adaptateurs (infrastructure) : Situés à l'extérieur, ils traduisent les échanges entre le monde extérieur et le domaine. Les adaptateurs primaires (ex: Controller Web) donnent des ordres au domaine, tandis que les adaptateurs secondaires (ex: un Repository) exécutent les demandes du domaine.
* La règle de dépendance vers l'intérieur : L'extérieur (l'infrastructure) dépend toujours de l'intérieur (le domaine), mais l'intérieur ignore totalement l'existence de l'extérieur.
...
**Réponse :** Comme les adaptateurs ont pour rôle de communiquer avec le monde extérieur (le réseau, la base de données, le framework Spring), de simples tests unitaires n'ont pas de sens, il faut tester l'intégration avec ces outils. 

 - Pour les adaptateurs primaires (Web / Controllers), on écrit des tests d'intégration Web, on "mock" (simule) le domaine, et on vérifie que le contrôleur gère bien les requêtes HTTP, les codes de retour, et la conversion du JSON.

 - Pour les adaptateurs secondaires (Persistance / Repositories), on écrit des tests d'intégration de base de données et on vérifie que le mapping fonctionne correctement et que les requêtes SQL s'exécutent bien sur une vraie base de données.
